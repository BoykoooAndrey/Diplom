...
for i in range(LEN_CAR):
    N_1_day[i] = round(N_1_year_all[i] / DAYS_JOB, 2)
    N_2_day[i] = round(N_2_year_all[i] / DAYS_JOB, 2)
    N_EO_smen_day[i] = round(N_EO_smen_year_all[i] / DAYS_JOB, 2)
    N_D1_day[i] = round(N_D1_year_all[i] / DAYS_JOB, 2)
    N_D2_day[i] = round(N_D2_year_all[i] / DAYS_JOB, 2)
...
k5_trud = form.find_k5_trud(weather, table_7)
...
for i in range(LEN_CAR):
    ...
    t_tr1000[i] = round(trud_normat[i][3] * k1_trud *
                        k2_trud[i] * k3_trud * k4_trud[i] * k5_trud, 2)

# 2.2 corrected trud
t_D1_corrected = [i for i in range(LEN_CAR)]
t_D2_corrected = [i for i in range(LEN_CAR)]
t_to1_corrected = [i for i in range(LEN_CAR)]
t_to2_corrected = [i for i in range(LEN_CAR)]
t_tr1000_corrected = [i for i in range(LEN_CAR)]

for i in range(LEN_CAR):
    t_D1_corrected[i] = round(0.25 * t_to1[i], 2)
    t_to1_corrected[i] = round(0.75 * t_to1[i], 2)
    t_D2_corrected[i] = round(0.1 * t_to2[i], 2)
    t_to2_corrected[i] = round((0.9 * t_to2[i]) * 1.2, 2)
    t_tr1000_corrected[i] = round(t_tr1000[i] * 0.9, 2)
...
T_to1_year_no_corrected_with_D = [i for i in range(LEN_CAR)]
T_to2_year_no_corrected_with_D = [i for i in range(LEN_CAR)]

for i in range(LEN_CAR):
    T_EO_year[i] = round(t_EO[i] * N_EO_smen_year_all[i], 2)
    T_D1_year[i] = round(t_D1_corrected[i] * N_D1_year_all[i], 2)
    T_D2_year[i] = round(t_D2_corrected[i] * N_D2_year_all[i], 2)
    T_to1_year[i] = round(t_to1_corrected[i] * N_1_year_all[i], 2)
    T_to2_year[i] = round(t_to2_corrected[i] * N_2_year_all[i], 2)
    T_tr1000_year[i] = round((Lg[i] * cars[i].quantity * t_tr1000_corrected[i]) / 1000, 2)
    T_to1_year_no_corrected_with_D[i] = round(t_to1[i] * N_1_year_all[i], 2)
    T_to2_year_no_corrected_with_D[i] = round(t_to2[i] * N_2_year_all[i], 2)

T_EO_year_all = sum(T_EO_year)
T_D1_year_all = sum(T_D1_year)
T_D2_year_all = sum(T_D2_year)
T_to1_year_all = sum(T_to1_year)
T_to2_year_all = sum(T_to2_year)
T_tr1000_year_all = sum(T_tr1000_year)
T_vsp = (T_EO_year_all + T_to1_year_all + T_to2_year_all + T_tr1000_year_all) * 0.2


def append_2():
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_EOc{i}')
        arr_value_output_1.append(t_EO[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_to1{i}')
        arr_value_output_1.append(t_to1[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_to2{i}')
        arr_value_output_1.append(t_to2[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_tr1000{i}')
        arr_value_output_1.append(t_tr1000[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_D1_corrected{i}')
        arr_value_output_1.append(t_D1_corrected[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_D2_corrected{i}')
        arr_value_output_1.append(t_D2_corrected[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_to1_corrected{i}')
        arr_value_output_1.append(t_to1_corrected[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_to2_corrected{i}')
        arr_value_output_1.append(t_to2_corrected[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f't_tr1000_corrected{i}')
        arr_value_output_1.append(t_tr1000_corrected[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_EO_year{i}')
        arr_value_output_1.append(T_EO_year[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_D1_year{i}')
        arr_value_output_1.append(T_D1_year[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_D2_year{i}')
        arr_value_output_1.append(T_D2_year[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_to1_year{i}')
        arr_value_output_1.append(T_to1_year[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_to2_year{i}')
        arr_value_output_1.append(T_to2_year[i])
    for i in range(LEN_CAR):
        arr_headings_output_1.append(f'T_tr1000_year{i}')
        arr_value_output_1.append(T_tr1000_year[i])
    arr_headings_output_1.append(f'T_EO_year_all')
    arr_value_output_1.append(T_EO_year_all)
    arr_headings_output_1.append(f'T_D1_year_all')
    arr_value_output_1.append(T_D1_year_all)
    arr_headings_output_1.append(f'T_D2_year_all')
    arr_value_output_1.append(T_D2_year_all)
    arr_headings_output_1.append(f'T_to1_year_all')
    arr_value_output_1.append(T_to1_year_all)
    arr_headings_output_1.append(f'T_to2_year_all')
    arr_value_output_1.append(T_to2_year_all)
    arr_headings_output_1.append(f'T_tr1000_year_all')
    arr_value_output_1.append(T_tr1000_year_all)
    arr_headings_output_1.append(f'T_vsp')
    arr_value_output_1.append(T_vsp)
append_2()
...
#№№№№№№№№№№№№№№№№№№№№№ Часть ДВА
# 3 X TO-1 X TO-2 X D
ritm_to_1 = round((60*modes_to_1[2]*modes_to_1[1])/N_1_day_sum, 2)
R_p_to_1 = 2
time_peredviz_on_to = 2
takt_to_1 = round((60*statistics.mean(t_to1_corrected)/R_p_to_1)+time_peredviz_on_to, 2)
X_to_1 =round(takt_to_1/ritm_to_1)
if X_to_1 == 0:
    X_to_1 = 1

ritm_to_2 = round((60*modes_to_2[2]*modes_to_2[1])/N_2_day_sum, 2)
R_p_to_2 = 3
n_to_2 = 2
takt_to_2 = round((60*statistics.mean(t_to2_corrected)/R_p_to_2)+time_peredviz_on_to, 2)
...
def append3(arr1, arr2):
    ...
    arr1.append(f't_to2_corrected')
    arr2.append(round(statistics.mean(t_to2_corrected),2))
    arr1.append(f'ritm_to_2')
    arr2.append(ritm_to_2)
...
R_p_D1 = 2
R_p_D2 = 1
X_d_1 = round(T_D1_year_all/ (DAYS_JOB  * modes_D[1] * modes_D[2] * n_d * R_p_D1))
X_d_2 = round(T_D2_year_all/ (DAYS_JOB  * modes_D[1] * modes_D[2] * n_d * R_p_D2))
if X_d_1 == 0:
    X_d_1 = 1
if X_d_2 == 0:
    X_d_2 = 1
...
X_EO_line = round((T_EO_year_all*fi_for_EO)/(DAYS_JOB*modes_EO[2]*n_2_for_EO[0]*modes_EO[1]*R_p_EO_line))
if X_EO_line == 0:
    X_EO_line = 1
...
fi_for_TR = form.find_fi('TR', all_quantity, modes_TR[1], table_24)
R_p_TR = 2
n_2_for_TR = form.find_n_2_for_TR(modes_TR[1], table_29)
X_tr = round((trud_post_all * fi_for_TR)/(DAYS_JOB*modes_TR[2]*R_p_TR*n_2_for_TR*modes_TR[1]))
if X_tr == 0:
    X_tr = 1
def append_3_2_17(arr1, arr2):
    arr_headings_output_2.append(f'trud_post_all')
    arr_value_output_2.append(trud_post_all)
    arr1.append(f'fi_for_TR')
    arr2.append(fi_for_TR)
...
X_to_1_v2 = X_to_1
f_a_to_1 = average_S
F_z_to_1 = round(X_to_1_v2 * f_a_to_1 * k_p_to_1)

k_p_to_2 = 5
X_to_2_v2 = X_to_2
f_a_to_2 = average_S
F_z_to_2 = round(X_to_2_v2 * f_a_to_2 * k_p_to_2)
...
F_z_TR = X_tr * average_S * k_p_d_tr
...
def append_3_2_20(arr1, arr2):
    arr1.append(f'X_to_1_v2')
    arr2.append(X_to_1_v2)
    arr1.append(f'f_a_to_1')
    arr2.append(f_a_to_1)
    arr1.append(f'k_p_to_1')
    arr2.append(k_p_to_1)

    arr1.append(f'X_to_2_v2')
    arr2.append(X_to_2_v2)
    arr1.append(f'f_a_to_2')
    arr2.append(f_a_to_2)
...
F_x = [i for i in range(LEN_CAR)]
for i in range(LEN_CAR):
    F_x[i] = S_cars[i]*cars[i].quantity*2
F_x_all = sum(F_x)
...
def table_1_1(tables):
    ...
    def append(fractions):
        ...
        for i in range(LEN_CAR):
            ...
            trud = 0
            for j in range(len(fractions[i])):
                tables.cell(row = j + count_row, column=(1)).value = fractions[i][j][0]
                tables.cell(row = j + count_row, column=(2 + i*3)).value = fractions[i][j][1]
                tables.cell(row = j + count_row, column=(3 + i*3)).value = fractions[i][j][2]
                trud_one_uch = 0
            ...
            tables.cell(row=(count_row + len(fractions[i])), column=(2 + i*3)).value = procent
            tables.cell(row=(count_row + len(fractions[i])), column=(3 + i*3)).value = trud
...
def table_1_5(tables):
    ...
    for i in range(len(f_1_f_2_for_regions)):
        if f_1_f_2_for_regions[i][4] != 0:
                F_reg = f_1_f_2_for_regions[i][2] + f_1_f_2_for_regions[i][3] * (f_1_f_2_for_regions[i][1] - 1)
...
def table_1_6(tables):
    ...
    for i in range(LEN_CAR):
        ...
        for j in range(len(f_sklad_k_4)):
            F_sklad = round(0.1 * cars[i].quantity * f_sklad_k_4[j][1] * k_1_sklad * k_2_sklad * k_3_sklad * f_sklad_k_4[j][2] * k_5_sklad)
            if F_sklad == 0:
                F_sklad = 1
            ...
            tables.cell(row=102+i, column=2+j).value = f"0.1⋅{cars[i].quantity}⋅{f_sklad_k_4[j][1]}⋅{k_1_sklad}⋅{k_2_sklad}⋅{k_3_sklad}⋅{f_sklad_k_4[j][2]}⋅{k_5_sklad}={F_sklad}"
...
F_z_all = F_z_to_1_okrug + F_z_to_2_okrug + F_z_D_okrug + F_z_EO_okrug + F_z_TR_okrug
...
for i in range(LEN_CAR):
    ...
    P_ud_etalon = round(standart[0] * k_1_standart[0] * k_2_standart[0] * k_3_standart[0] * k_4_standart[0]  * k_6_standart[0] * k_7_standart[0],2)
    X_ud_etalon = round(standart[1] * k_1_standart[1] * k_2_standart[1] * k_3_standart[1] * k_4_standart[1]  * k_6_standart[1] * k_7_standart[1], 2)
    S_prsk_etalon = round(standart[2] * k_1_standart[2] * k_2_standart[2] * k_3_standart[2] * k_4_standart[2]  * k_6_standart[2] * k_7_standart[2], 2)
    S_vsp_etalon = round(standart[3] * k_1_standart[3] * k_2_standart[3] * k_3_standart[3] * k_4_standart[3]  * k_6_standart[3] * k_7_standart[3], 2)
    S_st_etalon = round(standart[4] * k_2_standart[4] * k_3_standart[4] * k_5_standart, 2)
    S_t_etalon = round(standart[5] * k_1_standart[4] * k_2_standart[5] * k_3_standart[5] * k_4_standart[4] * k_6_standart[4] * k_7_standart[4], 2)
...
S_pr_skl_ne_correct = round((F_proiz_korp+F_sklad_vne_proizvod_korp)/all_quantity,2)
S_vsp_ne_correct = round((F_vsp+F_abk+F_kpp)/all_quantity,2)
S_st_ne_correct = round(sum(F_x)/all_quantity,2)


P_ud_srav = round((P_ud_ne_correct -  statistics.mean(P_ud_etalon_all)) * 100 /  statistics.mean(P_ud_etalon_all),2)
X_ud_srav = round((X_ud_ne_correct -  statistics.mean(X_ud_etalon_all)) * 100 / statistics.mean(X_ud_etalon_all),2)
S_prsk_srav = round((S_pr_skl_ne_correct -  statistics.mean(S_prsk_etalon_all)) * 100 / statistics.mean(S_prsk_etalon_all),2)
S_vsp_srav = round((S_vsp_ne_correct -  statistics.mean(S_vsp_etalon_all)) * 100 / statistics.mean(S_vsp_etalon_all),2)
S_st_srav = round((S_st_ne_correct -  statistics.mean(S_st_etalon_all)) * 100 / statistics.mean(S_st_etalon_all),2)


F_abk = statistics.mean(S_vsp_etalon_all) * all_quantity/4
F_gen_plan = round((F_proiz_korp + F_abk + F_x_all + F_kpp) / 0.45, 2)
S_t_ne_correct = round(F_gen_plan/all_quantity,2)
S_t_srav = round((S_t_ne_correct -  statistics.mean(S_t_etalon_all)) * 100 / statistics.mean(S_t_etalon_all),2)
...
arr_value_output_3.append(F_proiz_korp+F_sklad_vne_proizvod_korp)
